...
// 格式化，转当地时区时间
    ...
    /**
     * 泛型 extends  ，泛型 super  ：解决参数范围，向上或向下
     * <p>
     * 准则PECS： producer extends consumer super
     * 1. 频繁往外读取内容，适合用上界 extends
     * 2. 频繁往里插入，适合用下界 super
     */
    @Test
    public void generics() throws JsonProcessingException {
        ArrayList<Integer> integers = Lists.newArrayList(2, 1, 3);

        ArrayList<Double> doubles = Lists.newArrayList(6D, 4D, 5D);

        // 1. 虽然 Integer、Double 都是Number的子类，但是在调用  private void sort(List<Number> list){} 方法时，编译都会红线，参数类型不匹配
        // 2.  private void sort(List<? extends Number> list){} , 表示方法接受参数为，Number及其子类
        sort(integers);
        sort(doubles);
        System.out.println(integers);
        System.out.println(doubles);

        Filter<Number> numberFilter = new Filter<Number>() {
            @Override
            public boolean whether(Number number) {
        ...
        /*ObjectMapper objectMapper = new ObjectMapper();
        HashMap readValueMap = objectMapper.readValue(jsonString, HashMap.class);*/

        HashMap<String, Integer> readValueMap2 = formatJsonString(jsonString, new TypeReference<HashMap<String, Integer>>() {
        });
        System.out.println(readValueMap2.get("B"));

    }

    // private void sort(List<Number> list){
    private void sort(List<? extends Number> list) {
        // 1. 当 使用 ? extend 确定泛型范围后，使用add再次添加元素时，是无法编译通过的。
        // 2. 因为方法被调用时，无法确定传入的参数是Number的哪一个子类，所以JDK为了
        // list.add(7);
        if (CollectionUtils.isEmpty(list)) {
            return;
        }
        list.sort(Comparator.comparing(Number::intValue));
    }
    ...
    // private <E> void removeIf(List<E> list, Filter< E> filter) {
    private <E> void removeIf(List<E> list, Filter<? super E> filter) {
        list.removeIf(filter::whether);
    }
     ...
     * @param <T>
     */
    class TestGeneric<T extends Object & Serializable & Runnable & Comparator> {
    }

    public <T> T formatJsonString(String json, TypeReference<T> typeReference) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();
    ...
    public void generic2() {
        ...
        list5.add(new Fruit());
        list5.add(new Apple());
        // list5.add(new Food()); //报错

        List<? super Fruit> list6 = new ArrayList<Food>();
        list6.add(new Fruit());
        list6.add(new Apple());
        // list6.add(new Food());//报错
        ...
        List<? super Fruit> list8 = new ArrayList<Fruit>() {
            {
                add(new Fruit());
                add(new Apple());
                // add(new Food()); //报错, 上界为Fruit
                // add(new Meat()); //报错，上界为Fruit
            }
        };

        List<? super Food> list9 = new ArrayList<>();
        // 2. 实例化后添加对象，上界是定义时指定的T，list8定义时指定为Food。所以可以添加 new Food
        list9.add(new Food());
        list9.add(new Meat());
        list9.add(new Fruit());
        list9.add(new AppleSon());

        // get出来默认为 Object类型
        Object object = list9.get(0);
